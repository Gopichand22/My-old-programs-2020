...
public class LaboratoryApplication {
	...
	public static void main(String[] args) {
		...
		patientObject = new Patient[records];
		boolean isValid = true;
		do {
			switch (chooseOption()) {
			case 1:
				arrayCount = registerPatient(patientObject, arrayCount, records);
				break;
			case 2:
...
//patient registration 
	private static int registerPatient(Patient[] patientObject, int arrayCount, int records) {
		if (arrayCount < records) {
			...
			patientObject[arrayCount] = new Patient(name, age, gender, mobileNumber, address, docName, testName,
					commentsOnTest);
			System.out.println(Arrays.toString(patientObject));
			arrayCount++;
		} else {
